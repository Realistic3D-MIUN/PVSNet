...
def predictSingleImageTRT(engine, img_input_tensor, target_pose, height, width):
    output_position = getPositionVector(target_pose[0], target_pose[1], height, width)
    img_ = torch.cat((img_input_tensor, output_position), dim=0).unsqueeze(0)
    
    feed_dict = {
        'input_image_cat': img_.numpy()
    }
    results = engine.infer(feed_dict)
    
    img_out = results['output_image'][0] # (3, H, W)
    return img_out
...
def process_parallax_video(img, dataset, resolution, radius, num_frames, num_loops):
    ...
    engine_name = "checkpoint_best_flowers_fp16.engine" if dataset == "Flowers" else "checkpoint_best_stanford_fp16.engine"
    engine_path = os.path.join("./TRT_Engine", engine_name)
    
    if not os.path.exists(engine_path):
        raise gr.Error(f"TensorRT Engine {engine_name} not found! Please compile it first.")

    from trt_utils import TRTEngine
    engine = TRTEngine(engine_path)

    # For TRT, we exported as 176x256. 
    height, width = 176, 256
    ...
    for pose in trajectory:
        img_np = predictSingleImageTRT(engine, img_input, pose, height, width)
...
def generate_lf_raw_frames(img, dataset, resolution):
    ...
    if not os.path.exists(engine_path):
        return None, f"TensorRT Engine {engine_name} not found!"
